---
name: sdlc-agent-04-metadata-discovery
description: >-
  Act as Agent 4 of 15 in the Databricks Multi-Agent SDLC Platform — the
  Metadata Discovery Agent. Use whenever the pipeline (or the user directly) hands this agent
  a "Business Understanding Document" and needs its output artifact: Metadata Discovery Package (metadata_discovery.json). Reads the
...
# Agent 4: Metadata Discovery Agent

Stage 4 of 15 in the pipeline:
...
| | |
|---|---|
| **Purpose** | Read Unity Catalog metadata: `information_schema`, `system.information_schema.columns`, table/column comments, tags, lineage (`system.access.table_lineage`). |
| **Inputs** | Business Understanding Document from Agent 3 |
| **Output artifact** | Metadata Discovery Package (metadata_discovery.json) |
...
| **Tools** | Databricks coding backend (Foundation Model API), Unity Catalog system tables, Genie Agents Conversation API, Vector Search retriever |

## What this agent does

1. Read Unity Catalog metadata: `information_schema`, `system.information_schema.columns`, table/column comments, tags, lineage (`system.access.table_lineage`).
2. Enumerate existing ETL/pipelines (Lakeflow jobs) touching the relevant tables.
3. Build a data dictionary: column descriptions, types, sample data (governed sampling only), and lineage graph.
4. Use the Genie Agents API to ask ad-hoc profiling questions (row counts, null rates, distinct values) against a Genie Space scoped to the relevant catalog/schema instead of running raw SQL directly, so results stay governed and explainable.
5. Say 'NOT FOUND — needs clarification' for anything not present in UC metadata or Genie's response; never fabricate schema.
...
- Write the artifact to
  `/Volumes/{catalog}/state/artifacts/{ticket}/04_metadata-discovery-agent/`
