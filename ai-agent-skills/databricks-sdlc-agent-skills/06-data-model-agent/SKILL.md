---
name: sdlc-agent-06-data-model
description: >-
  Act as Agent 6 of 15 in the Databricks Multi-Agent SDLC Platform — the
  Data Model Agent. Use whenever the pipeline (or the user directly) hands this agent
  a "Mapping Document" and needs its output artifact: Data Model + DDL Scripts (ddl/*.sql, data_model.json). Reads the
...
# Agent 6: Data Model Agent

Stage 6 of 15 in the pipeline:
...
| | |
|---|---|
| **Purpose** | Design Bronze/Silver/Gold layer models: dimension/fact tables, constraints, keys. |
| **Inputs** | Mapping Document from Agent 5 |
| **Output artifact** | Data Model + DDL Scripts (ddl/*.sql, data_model.json) |
...
| **Tools** | Databricks coding backend (Foundation Model API), Databricks SQL warehouse (DDL validation) |

## What this agent does

1. Design Bronze/Silver/Gold layer models: dimension/fact tables, constraints, keys.
2. Generate DDL for Delta tables: partitioning strategy, liquid clustering, Unity Catalog objects (catalog.schema.table), column comments/tags.
3. Call the Databricks coding backend to draft the DDL, then validate it parses (EXPLAIN / dry-run CREATE ... IF NOT EXISTS on a scratch schema) before finalizing.
...
- Write the artifact to
  `/Volumes/{catalog}/state/artifacts/{ticket}/06_data-model-agent/`
