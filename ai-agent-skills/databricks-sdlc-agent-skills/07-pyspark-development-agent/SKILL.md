---
name: sdlc-agent-07-pyspark-development
description: >-
  Act as Agent 7 of 15 in the Databricks Multi-Agent SDLC Platform — the
  PySpark Development Agent. Use whenever the pipeline (or the user directly) hands this agent
  a "Data Model + DDL" and needs its output artifact: Production-ready PySpark notebooks (`# Databricks notebook source` format). Reads the
...
# Agent 7: PySpark Development Agent

Stage 7 of 15 in the pipeline:
...
| | |
|---|---|
| **Purpose** | Generate metadata-driven, incremental PySpark: Delta MERGE/SCD logic, error handling, logging & audit, reusable components — following the medallion framework conventions if that skill is installed. |
| **Inputs** | Data Model + DDL from Agent 6, Mapping Document from Agent 5 |
| **Output artifact** | Production-ready PySpark notebooks (`# Databricks notebook source` format) |
...
| **Tools** | Databricks coding backend (Foundation Model API), Jobs API (smoke-test run) |

## What this agent does

1. Generate metadata-driven, incremental PySpark: Delta MERGE/SCD logic, error handling, logging & audit, reusable components — following the medallion framework conventions if that skill is installed.
2. Call the Databricks coding backend (Foundation Model API chat-completions) with the full contract: mapping doc + DDL + medallion conventions as context, requiring code-only output plus a short rationale.
3. Execute the generated notebook on a scratch job cluster / serverless compute as a smoke test before marking the contract complete; on failure, do ONE repair retry with the stack trace, then route to human.
...
- Write the artifact to
  `/Volumes/{catalog}/state/artifacts/{ticket}/07_pyspark-development-agent/`
