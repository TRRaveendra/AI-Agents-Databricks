---
name: sdlc-agent-09-data-quality
description: >-
  Act as Agent 9 of 15 in the Databricks Multi-Agent SDLC Platform — the
  Data Quality Agent. Use whenever the pipeline (or the user directly) hands this agent
  a "PySpark code" and needs its output artifact: DQ rules & expectations + reports (dq_rules.json, dq_report.md). Reads the
...
# Agent 9: Data Quality Agent

Stage 9 of 15 in the pipeline:
...
| | |
|---|---|
| **Purpose** | Derive null/duplicate checks, business-rule validation, referential integrity, completeness and freshness expectations from the data model and mapping doc. |
| **Inputs** | PySpark code from Agent 7, SQL from Agent 8, Data Model from Agent 6 |
| **Output artifact** | DQ rules & expectations + reports (dq_rules.json, dq_report.md) |
...
| **Tools** | Databricks coding backend (Foundation Model API), SQL warehouse (rule execution) |

## What this agent does

1. Derive null/duplicate checks, business-rule validation, referential integrity, completeness and freshness expectations from the data model and mapping doc.
2. Emit Lakeflow/DLT-style expectations (`@dlt.expect_*`) or a DQX/Great-Expectations-style rules file, matching whatever DQ framework the workspace standardizes on.
3. Run the rules against a sample/scratch table and produce a DQ score + report.
...
- Write the artifact to
  `/Volumes/{catalog}/state/artifacts/{ticket}/09_data-quality-agent/`
