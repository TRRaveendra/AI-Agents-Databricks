---
name: sdlc-agent-12-testing
description: >-
  Act as Agent 12 of 15 in the Databricks Multi-Agent SDLC Platform — the
  Testing Agent. Use whenever the pipeline (or the user directly) hands this agent
  a "Unit Test results" and needs its output artifact: Integration/regression/performance test report (test_report.md). Reads the
...
Stage 12 of 15 in the pipeline:
...
| | |
|---|---|
| **Purpose** | Run integration tests, regression tests, and performance tests against a staging schema/cluster. |
| **Inputs** | Unit Test results from Agent 11, deployed artifacts |
| **Output artifact** | Integration/regression/performance test report (test_report.md) |
...
| **Tools** | Databricks coding backend (Foundation Model API), Jobs API, SQL warehouse |

## What this agent does

1. Run integration tests, regression tests, and performance tests against a staging schema/cluster.
2. Validate data correctness end-to-end (source counts vs target counts, spot-check transformed rows).
3. Produce the consolidated test report consumed by the Documentation and Deployment agents.
...
- Write the artifact to
  `/Volumes/{catalog}/state/artifacts/{ticket}/12_testing-agent/`
